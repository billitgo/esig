...
- **盖章记录列表已上线**（2026-09-04，feature/stamp-records 分支）：左侧面板"盖章记录: 共(N条)记录"区块，每条=页码+盖章时间(yyyy-MM-dd HH:mm:ss)+红色删除按钮（删除同步移除印章）；数据来源 stampAt 新增的 `createdAt` 字段（仅加字段，未改盖章/骑缝章行为）；删除走 App.vue#deleteRecord(id)
- **git 已初始化**：main=基线提交，feature/stamp-records=盖章记录列表（当前工作分支）
- 用户浏览器环境下载会被自动化拦截，导出逻辑本身正常（此前像素级验证通过）
- 当前服务以 `npm start` / `npm run dev` 后台运行于 3000/5173
